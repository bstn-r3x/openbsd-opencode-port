...

{
  ...
  strings -a "$BIN_PATH" 2>/dev/null \
    | egrep '(@opentui/core-openbsd-x64/index.ts|libopentui\\.so|/srv/opencode-port|\\.bun/@opentui\\+core@)' \
    | head -80 || true

  echo
  echo '-- portability flags --'
  if strings -a "$BIN_PATH" 2>/dev/null | grep -q '/srv/opencode-port'; then
    echo 'contains_private_workspace_paths=yes'
  else
    echo 'contains_private_workspace_paths=no'
  fi
  if strings -a "$BIN_PATH" 2>/dev/null | grep -q '@opentui/core-openbsd-x64/index.ts'; then
